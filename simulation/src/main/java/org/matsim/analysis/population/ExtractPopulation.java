...
//        path for super computing
        while (threshold <= 8) {
            URL popFloodsPath = ExtractPopulation.class.getResource("/nanjing/outputPlans/output_plans_threshold" + threshold + "m.xml.gz");
            URL popBaselinePath = ExtractPopulation.class.getResource("/nanjing/outputPlans/Sampled_plans_baseline.xml");
    ...
    public static Population run(Population sampledPopBaseline, Population popFloods) {
        ...
        for (Person person : popFloods.getPersons().values()) {
            if (sampledPopBaseline.getPersons().containsKey(person.getId())) {
                // Note, we should filter with key (personID) not value (personID, personTrip and so on).
